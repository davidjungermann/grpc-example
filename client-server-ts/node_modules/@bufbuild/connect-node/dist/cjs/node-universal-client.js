...
function createNodeHttpClient(options) {
    ...
    return options.httpVersion == "2"
        ? createNodeHttp2Client(options.baseUrl, (_a = options.keepSessionAlive) !== null && _a !== void 0 ? _a : false, options.nodeOptions)
...
function createNodeHttp2Client(authority, keepSessionOpen, http2SessionOptions) {
    ...
    return async function request(req) {
        ...
        return new Promise((resolve, reject) => {
            ...
            h2Request(sentinel, sessionHolder, req.url, req.method, (0, node_universal_header_js_1.webHeaderToNodeHeaders)(req.header), {}, (stream) => {
...
function h2Request(sentinel, sessionHolder, url, method, headers, options, onStream) {
    ...
    function h2ConnectedSession(session) {
        ...
        const stream = session.request(Object.assign(Object.assign({}, headers), { ":method": method, ":path": requestUrl.pathname + requestUrl.search }), options);
        sentinel
            .catch((reason) => {
            if (stream.closed) {
                return;
