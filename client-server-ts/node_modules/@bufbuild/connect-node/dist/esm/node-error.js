...
export function connectErrorFromH2ResetCode(rstCode) {
    switch (rstCode) {
        ...
        case H2Code.CONNECT_ERROR:
            return new ConnectError(`http/2 stream closed with RST code ${H2Code[rstCode]} (0x${rstCode.toString(16)})`, Code.Internal);
        case H2Code.REFUSED_STREAM:
            return new ConnectError(`http/2 stream closed with RST code ${H2Code[rstCode]} (0x${rstCode.toString(16)})`, Code.Unavailable);
        case H2Code.CANCEL:
            return new ConnectError(`http/2 stream closed with RST code ${H2Code[rstCode]} (0x${rstCode.toString(16)})`, Code.Canceled);
        case H2Code.ENHANCE_YOUR_CALM:
            return new ConnectError(`http/2 stream closed with RST code ${H2Code[rstCode]} (0x${rstCode.toString(16)})`, Code.ResourceExhausted);
        case H2Code.INADEQUATE_SECURITY:
            return new ConnectError(`http/2 stream closed with RST code ${H2Code[rstCode]} (0x${rstCode.toString(16)})`, Code.PermissionDenied);
